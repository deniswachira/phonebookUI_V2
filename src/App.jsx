...
function App() {
  ...
  useEffect(() => {
    const fetchContact = async () => {
      try {
        const res = await axios.get(`${ConstantURL}/api`).then((response) => {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    axios.post(`${ConstantURL}/api/contacts`, { name, phonenumber })
      .then(res => {
  ...
  const handleDelete = async (_id) => {
    try {
      await axios.delete(`${ConstantURL}/api/contacts/${_id}`);
